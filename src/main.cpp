...
int main()
{
    ...
    Texture3D texture;
    int size = 128;
    texture.width = texture.height = texture.depth = size;
    
    std::vector<unsigned char> data1(size * size * size, 127);
    ...
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RED, size, size, size, 0, 
                 GL_RED, GL_UNSIGNED_BYTE, data1.data());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ourShader.setMat4("inverseProjection", inverseProjection);
